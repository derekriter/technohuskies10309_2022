...
//            frontRight.setDirection(DcMotorSimple.Direction.FORWARD);
    ...
    public void driveOdo(float inches, float speed) {
        ...
        Orientation angles = this.hardware.getIMU().getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.XYZ, AngleUnit.DEGREES);
        ...
        while(Math.abs(enc.getCurrentPosition() - target) > precision && this.opMode.opModeIsActive()) {
            ...
            angles = this.hardware.getIMU().getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.XYZ, AngleUnit.DEGREES);
            err = enc.getCurrentPosition() - target;
            aErr = 0 - angles.secondAngle;

            this.opMode.telemetry.addData("Angle Error", aErr);

            double corr = calcPID(err, trend, Kp, Ki, Kd) * multiplier;
            double turnpid = calcPID(aErr, a_trend, aKp, aKi, aKd) * aMultiplier;
...
//        this.turn((float) -aErr, 0.1f, 1);
    ...
    //
    //ODOMETRY SUBFUNCTIONS
    //

    private double calcPID(double error, ArrayList<Double> trend, double Kp, double Ki, double Kd) {
        double I = trend.stream().reduce(0d, Double::sum);
        double D = -trend.get(trend.size() - 1) + error;
